...
function createBaseUser(input: InsertUser & { id?: number }): User {
  ...
  return {
    ...
    serviceMode: input.serviceMode ?? "single",
    servedCityIdsJson: input.servedCityIdsJson ?? null,
    deliveryCityIdsJson: input.deliveryCityIdsJson ?? null,
...
export function upsertLocalUser(input: InsertUser) {
  ...
  const updated: User = {
    ...
    servedCityIdsJson: input.servedCityIdsJson ?? existing.servedCityIdsJson,
    deliveryCityIdsJson: input.deliveryCityIdsJson ?? existing.deliveryCityIdsJson,
